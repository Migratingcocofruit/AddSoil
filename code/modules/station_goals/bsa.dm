...
// Safe BSA. Doesn't destroy things.

/obj/machinery/bsa/full/safe

/obj/machinery/bsa/full/safe/fire(mob/user, turf/bullseye, target)
	var/turf/point = get_front_turf()
	point.Beam(get_target_turf(), icon_state = "bsa_beam", time = 50, maxdistance = world.maxx, beam_type = /obj/effect/ebeam) //fake ZZZAP
	playsound(src, 'sound/machines/bsa_fire.ogg', 100, 1)

	reload()

/obj/item/circuitboard/computer/bsa_control/safe
	build_path = /obj/machinery/computer/bsa_control/safe
	board_name = "Bluespace Artillery Safe Controls"

/obj/machinery/computer/bsa_control/safe
	name = "Bluespace Artillery Safe Control"
	circuit = /obj/item/circuitboard/computer/bsa_control/safe

/obj/machinery/computer/bsa_control/safe/deploy()
	var/obj/machinery/bsa/full/prebuilt = locate() in range(7, src) //In case of adminspawn
	if(prebuilt)
		prebuilt.controller = src
		return prebuilt
	...
	s.set_up(4, FALSE, centerpiece)
	s.start()
	var/obj/machinery/bsa/full/safe/cannon = new(get_turf(centerpiece),centerpiece.get_cannon_direction())
	cannon.controller = src
	qdel(centerpiece.front)
	qdel(centerpiece.back)
	qdel(centerpiece)
